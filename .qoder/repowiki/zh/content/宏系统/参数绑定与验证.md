...
graph TB
subgraph "宏系统"
M1["宏处理器<br/>参数类型判定与绑定"]
MU["工具扩展<br/>类型简化"]
end
subgraph "运行时"
R1["HttpRequest<br/>解析请求头/URL/Body"]
R2["HttpResponse<br/>响应构造"]
S1["Server<br/>路由注册与OpenAPI生成"]
SA["ServerAuth<br/>JWT签发/校验"]
end
subgraph "示例"
E1["示例服务器<br/>参数绑定演示"]
end
...
participant H as "宏生成的包装函数"
participant R as "HttpRequest"
participant F as "处理器函数"
C->>S : "HTTP请求"
S->>R : "解析请求头/URL/Body"
R-->>S : "填充body_pairs/body_files/url_query等"
S->>H : "按路由匹配调用包装函数"
H->>H : "参数绑定与类型转换"
H->>F : "调用处理器函数"
F-->>H : "返回结果或错误"
...
```mermaid
flowchart TD
Start(["进入宏生成的包装函数"]) --> CheckType["判断参数类型"]
CheckType --> IsPostFile{"是否PostFile?"}
IsPostFile --> |是| GetFile["从body_files按参数名取文件"]
GetFile --> FileExists{"是否存在?"}
FileExists --> |否| ErrMissFile["返回错误：缺少参数"]
FileExists --> |是| NextArg["下一个参数"]
IsPostFile --> |否| IsScalar{"是否基础标量类型?"}
IsScalar --> |否| ErrUnsupported["编译期报错：不支持的类型"]
IsScalar --> |是| BindScalar["绑定逻辑"]
BindScalar --> FromPairs["先从body_pairs取值"]
FromPairs --> FoundPairs{"找到?"}
FoundPairs --> |是| NextArg
FoundPairs --> |否| FromQuery["再从url_query取值"]
FromQuery --> FoundQuery{"找到?"}
FoundQuery --> |否| ErrMissing["返回错误：缺少参数"]
FoundQuery --> |是| NextArg
NextArg --> Done{"还有参数?"}
Done --> |是| CheckType
Done --> |否| ConvertParse["非String类型尝试parse()"]
ConvertParse --> ParseOk{"转换成功?"}
ParseOk --> |否| ErrType["返回错误：参数类型不匹配"]
ParseOk --> |是| CallHandler["调用处理器函数"]
CallHandler --> WrapResp["按返回类型封装HttpResponse"]
WrapResp --> End(["结束"])
```
...
```mermaid
flowchart TD
Enter(["进入请求体解析"]) --> ReadCT["读取Content-Type"]
ReadCT --> CTMatch{"匹配类型?"}
CTMatch --> |JSON| ParseJSON["解析JSON对象<br/>写入body_pairs"]
CTMatch --> |x-www-form-urlencoded| ParseForm["按&和=分割<br/>写入body_pairs"]
CTMatch --> |multipart/form-data| ParseMultipart["按boundary切分<br/>解析name/filename"]
ParseMultipart --> HasFilename{"有filename?"}
HasFilename --> |是| SaveFile["写入body_files"]
HasFilename --> |否| SavePair["写入body_pairs"]
ParseJSON --> Done(["完成"])
ParseForm --> Done
SaveFile --> Done
SavePair --> Done
...
```mermaid
flowchart TD
Gen(["生成OpenAPI路径项"]) --> CollectArgs["收集处理器参数类型"]
CollectArgs --> Method{"HTTP方法"}
Method --> |GET| ExportQuery["导出parameters为query"]
Method --> |其他| ExportBody["导出requestBody为multipart/form-data"]
ExportQuery --> Done(["完成"])
ExportBody --> Done
```
...
C->>S : "携带Authorization : Bearer <token>"
S->>H : "进入包装函数"
H->>SA : "jwt_check(token)"
SA-->>H : "返回payload或错误"
alt 校验失败
H-->>C : "返回认证失败"
else 校验成功
...
```mermaid
graph LR
Macro["宏处理器"] --> Types["基础标量类型集合"]
Macro --> Utils["类型简化工具"]
Macro --> RuntimeTypes["运行时类型"]
Runtime["运行时"] --> Parser["请求解析"]
Runtime --> OpenAPI["OpenAPI生成"]
Runtime --> Auth["ServerAuth"]
```
