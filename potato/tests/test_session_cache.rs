...
async fn test_session_cache_token_generation() -> anyhow::Result<()> {
    // 测试 token 签发和解析
    SessionCache::set_jwt_secret(b"test-secret-key");

    let token = SessionCache::generate_token(12345, Duration::from_secs(3600))?;
    assert!(!token.is_empty());

    let (user_id, ttl) = SessionCache::parse_token(&token).expect("Token should be valid");
...
async fn test_session_cache_from_token() -> anyhow::Result<()> {
    ...
    let token = SessionCache::generate_token(99999, Duration::from_secs(3600))?;
    let cache = SessionCache::from_token(&token).expect("Should create session cache");
...
async fn test_session_cache_cross_request() -> anyhow::Result<()> {
    ...
    // 生成token
    let token = SessionCache::generate_token(77777, Duration::from_secs(3600))?;

    // 直接测试 SessionCache 的跨请求能力
    let cache1 = SessionCache::from_token(&token).expect("Should create cache");
    cache1.set("request_count", 1u32);

    let cache2 = SessionCache::from_token(&token).expect("Should get same cache");
    ...
    let cache3 = SessionCache::from_token(&token).expect("Should get same cache");
...
async fn test_session_cache_expiration() -> anyhow::Result<()> {
    ...
    // 生成一个只有2秒有效期的token
    let token = SessionCache::generate_token(88888, Duration::from_secs(2))?;
...
async fn test_session_cache_cleanup() -> anyhow::Result<()> {
    ...
    // 创建几个session
    let token1 = SessionCache::generate_token(11111, Duration::from_secs(1))?; // 1秒过期
    let token2 = SessionCache::generate_token(22222, Duration::from_secs(3600))?; // 1小时过期

    let _ = SessionCache::from_token(&token1);
    let _ = SessionCache::from_token(&token2);

    // 等待第一个session过期
    sleep(Duration::from_secs(2)).await;

    // 第一个token已过期，parse_token应该返回Error
    let parsed1 = SessionCache::parse_token(&token1);
    assert!(parsed1.is_err());

    // 第二个token仍然有效
    let parsed2 = SessionCache::parse_token(&token2);
...
async fn test_session_cache_different_users() -> anyhow::Result<()> {
    ...
    // 为不同用户生成token
    let token1 = SessionCache::generate_token(10001, Duration::from_secs(3600))?;
    let token2 = SessionCache::generate_token(10002, Duration::from_secs(3600))?;

    let cache1 = SessionCache::from_token(&token1).expect("User 1 cache");
    let cache2 = SessionCache::from_token(&token2).expect("User 2 cache");
